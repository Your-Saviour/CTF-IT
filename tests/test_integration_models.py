...
def test_only_one_active_job_is_allowed_per_binding():
    engine = create_engine("sqlite://")
    Base.metadata.create_all(engine)
    with Session(engine) as db:
        ...
        destination = IntegrationDestination(
            name="Expo", adapter_key="expo_it", base_url="https://expo.example",
        ...
        db.add_all([
            IntegrationSyncJob(binding_id=binding.id, status="pending", trigger_reason="one", next_attempt_at=utcnow()),
            IntegrationSyncJob(binding_id=binding.id, status="retrying", trigger_reason="two", next_attempt_at=utcnow()),
        ])
        with pytest.raises(IntegrityError):
            db.commit()
