...
test('filters modules required by others using the complete catalogue',()=>assert.deepEqual(filterModules(modules,{relationship:'required_by_others'}).map(m=>m.id),['app']));
test('filters modules that require no dependencies',()=>assert.deepEqual(filterModules(modules,{relationship:'no_requirements'}).map(m=>m.id),['app','other']));
test('combines relationship difficulty and category filters',()=>assert.deepEqual(filterModules(modules,{relationship:'no_requirements',difficulty:'easy',category:'containers'}).map(m=>m.id),['app']));
test('filters both compatible and incompatible modules for a base',()=>{assert.deepEqual(filterModules(modules,{compatibility:'compatible',baseType:'ubuntu'}).map(m=>m.id),['app','vuln']);assert.deepEqual(filterModules(modules,{compatibility:'incompatible',baseType:'ubuntu'}).map(m=>m.id),['other'])});
