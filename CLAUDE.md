...
- **Run endpoints** (admin router `/admin/api`): `POST /admin/events/{event_id}/operations/{operation_id}/run` (launches one run per event team, or a single team via `team_id`), `GET .../runs` (list), `GET /admin/api/operation-runs/{run_id}` (run + step detail), `POST /admin/api/operation-runs/{run_id}/steps/{step_id}/approve|reject` (instructor approval), `POST /admin/api/operation-runs/{run_id}/cancel`. Run detail page: `/admin/events/{event_id}/operations/{operation_id}/runs/{run_id}`.
