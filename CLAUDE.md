...
- **`POST /admin/caldera-setup`** — Fully automated: generates the plugin, copies it to the shared bind mount, patches `local.yml` to enable the `ctf-exploit` plugin, restarts the Caldera container, waits for health, waits for an agent belonging to an active VM in the event (matched by IP, not just group membership), creates a "CTF Red Team Emulation" operation, and caches attack tree JSON on each VM in the event. Event-based setup builds per-VM attack-path adversaries when VMs have module assignments (via `generate_caldera_event_export`); quota-only setup falls back to flat generation.
 - **`builder/caldera.py`** — Generates the plugin: iterates selected modules, creates one ability YAML per module (recon + exploit steps). Three generation modes: `generate_caldera_export()` (flat, single "CTF Full Exploit Chain" adversary), `generate_caldera_export_multi_path()` (per-path adversaries), and `generate_caldera_event_export()` (per-VM attack-path adversaries built from per-VM module assignments). Shared helpers: `_write_plugin()` (hook + abilities + adversaries + objectives + payloads/uploads staging + plugin `app/` staging), `_ability_requirements()`/`_ability_parsers()` (normalize module YAML metadata), `_default_recon_parsers()`/`_default_exploit_requirement()`/`_gate_exploit_command()`/`fact_trait()`, `_build_objectives()`/`_goal_entry()`/`goal_fact_trait()`/`objective_uuid()`. Exploit `uploads` are prefixed, substituted into the command, rendered in the ability YAML, and copied into the plugin payloads dir.

**Native fact gating (no shell markers):** recon→exploit ordering is enforced natively by Caldera facts + requirements instead of `/tmp/.ctf_phase_{N}` marker files. Every recon ability whose command emits the `VULNERABLE` marker gets a default parser (`plugins.ctf-exploit.app.parsers.ctf_basic`) emitting a `ctf.vuln.<module_id>` fact. Every exploit ability of a module with a recon step gets (a) a requirement (`plugins.ctf-exploit.app.requirements.fact_present`) on that fact and (b) a `#{ctf.vuln.<module_id>}` variable reference in its command, so Caldera's planner trims the link until recon confirms the vulnerability (requirements are only enforced for commands containing `#{...}` facts). Goal exploits (no recon) run unconditionally. Modules can override the defaults by declaring `parser`/`requirements` explicitly in their caldera YAML.

**Native objectives (goal achievement detection):** goal exploit abilities auto-get a parser emitting a `ctf.goal.<goal_id>` fact when output contains the `GOAL_ACHIEVED` marker (installed regardless of whether the marker is in the command string or a payload script's stdout, e.g. `install_c2`). `_build_objectives()` writes a per-goal objective plus a combined "CTF Objectives" objective into `plugins/ctf-exploit/data/objectives/`; each goal uses `operator: "*"` + `count: 1` so any fact with the goal's trait satisfies it. The master/per-tactic adversaries are wired to the combined objective; per-path adversaries terminating at a goal module are wired to that goal's objective (via the `objective:` adversary field). `objective_uuid()` is deterministic (UUIDv5 over sorted goal ids). Caldera's objective `percentage`/`completed` therefore track red team goal achievement natively.
...
- **Planner selection & human-in-loop controls**: `GET /admin/api/caldera/planners` lists available planners; `POST /admin/api/caldera/operations` accepts `planner_name` (default `atomic`), `autonomous` (bool), and `state`. `PATCH /admin/api/caldera/operations/{op_id}` updates `state` (running/paused/run_one_link/finished), `autonomous`, `jitter`, `obfuscator`, `visibility`; `PATCH /admin/api/caldera/operations/{op_id}/links/{link_id}` approves (`action: approve` → status `-3` EXECUTE) or rejects (`action: reject` → status `-2` DISCARD) a pending link. Non-autonomous operations create links at status `-1` (PAUSE) that the admin approves one at a time in the dashboard's "Pending Approvals" panel. Link status codes in `api/services/caldera.py`: `LINK_STATUS_PAUSE=-1`, `LINK_STATUS_EXECUTE=-3`, `LINK_STATUS_DISCARD=-2`.
- **Stealth & debrief**: `GET /admin/api/caldera/obfuscators` lists available obfuscators; `POST /admin/api/caldera/operations` accepts `obfuscator` (default `plain-text`), `jitter` (default `2/8`), and `visibility` (default 50). `GET /admin/api/caldera/operations/{op_id}/report?output=true` returns a finished operation's debrief report (per-agent steps, facts, objectives, jitter). The operation detail dashboard shows Pause/Step/Resume/Enable-Auto controls while running, a "Debrief" button when finished, and a pending-approvals panel for manual operations.
- **Event lifecycle integration**: `POST /admin/events/{id}/stop` and `DELETE /admin/events/{id}` automatically delete all Caldera operations in the `event-{id}` group to prevent orphaned operations. Running/paused operations are first transitioned to `finished` (via `PATCH /operations/{id}`) so their background planner loops exit cleanly before deletion; `POST /admin/api/caldera/operations/cleanup-orphaned` applies the same stop-then-delete treatment to operations whose event no longer exists.
...
Compiled operation plans can be executed as orchestrated sweeps against Caldera: each graph node runs as its own single-ability operation, and facts emitted by one step gate the next.

- **Models** (`api/models.py`): `OperationRun` — per team × operation execution (`event_id`, `operation_id`, `team_id`, `status` queued/running/completed/failed/cancelled, frozen `plan_snapshot`, `fact_store` JSON, `trigger`); `OperationRunStep` — per graph node (`node_id`, `node_type`, status/result, `output`, `attempts`, `caldera_operation_id`). Migration `0016_operation_runs`.
- **Run endpoints** (admin router `/admin/api`): `POST /admin/api/events/{event_id}/operations/{operation_id}/run` (launches one run per event team, or a single team via `team_id`), `GET /admin/api/events/{event_id}/operations/{operation_id}/runs` (list), `GET /admin/api/operation-runs/{run_id}` (run + step detail), `POST /admin/api/operation-runs/{run_id}/steps/{step_id}/approve|reject` (instructor approval), `POST /admin/api/operation-runs/{run_id}/cancel`. Run detail page: `/admin/events/{event_id}/operations/{operation_id}/runs/{run_id}`.
- **`api/services/operation_runner.py`**: `launch_run` compiles the frozen `plan_snapshot` and loops `next_ready_nodes` (from `builder/operation_compiler.py`); `_run_node` skips ability nodes whose `inputs` are missing from the fact store (`decide_node_execution`), pauses at `awaiting_approval` when the policy sets `instructor_approval` (`_wait_for_approval`), resolves the target VM via `_resolve_target_vm`, extracts emitted facts into `fact_store`, and finalizes the run. `mark_interrupted_runs` fails orphaned runs at startup.
- **`api/services/operation_driver.py`**: `OperationDriver` executes a single ability per step via pre-generated single-ability adversaries (`builder/caldera.py` `build_single_ability_adversaries`), a per-run fact source `ctf-run-{run_id}`, and `allowed_agents` for strict single-paw targeting.
- **Fact contract** (`builder/fact_contract.py`): `outputs`/`inputs` under `caldera.recon`/`caldera.exploit`; module-scoped trait naming (`ctf.<module_id>.<name>`); auto-derived `ctf.vuln.<module_id>` (recon `VULNERABLE`) and `ctf.goal.<goal_id>` (goal `GOAL_ACHIEVED`); `marker`/`pattern`/`group` semantics; `extract_facts`/`substitute_command` utilities; `validate_module_facts`/`validate_catalogue_facts` wired into catalogue validation. See MODULE_GUIDE.md → "Outputs and Inputs (Exploit Chaining)".
- **Showcase**: `seed_showcase_event` (startup) seeds a DRAFT "Operation Chaining Demo" event with a pre-authored "RCE → Privilege Escalation → Implant" chain (`weak_ssh_credentials` → `nopasswd_sudo` → `malicious_cron_beacon`).
- **Integration test**: `tests/test_operation_sweep_integration.py` is a Caldera smoke test, `@pytest.mark.skipif`-skipped unless `CALDERA_INTERNAL_URL` is set (exercises the single-ability driver round-trip); it always collects cleanly in the CI/Docker test service.
...
Vultr FreeBSD 15 resolution, official bootstrap download and SHA recording,
unattended conversion, two disk-boot gates, sanitization, snapshot/clone
validation, activation, and cleanup. Persistent
...
There is no ISO sidecar or console installation. `CTF_CONTROL_PLANE_CIDR` is
validated before creating temporary Vultr firewall resources. Cleanup failure after validation leaves the
